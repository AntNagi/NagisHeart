...
### TASK-20260726-002
- 标题：Web 系统页暗层同步 MinSpec §1 现行值（"看不见/看不清"总根因）
- 负责人：Wewe（Web）
- 状态：ready
- 优先级：P0
- 说明：feibo 2026-07-26 浏览器实测定性。`web/styles/tokens.css` 的 `--overlay-system-1` 仍是 07-21 修订**前的废止旧值** `rgba(19,32,51,0.32)→0.12@42%→0.66`，且**完全缺少 MinSpec §1 第二层径向暗角**。现行权威值：第一层 `0.52→0.34@42%→0.78`、第二层 `radial-gradient(ellipse at 50% 38%, transparent 0 18%, rgba(19,32,51,0.38) 62%, rgba(19,32,51,0.72) 100%)`、第三层白色高光不变。成因：90 项对齐（`e728137`）做在 MinSpec §1 修订之前，权威更新后无人回归同步。
- 影响（Ant 07-25 反馈中的多条同源）：系统页几乎不压暗 → 白色返回按钮/次要文字糊在亮发丝背景上看不见（对应她第 5 条"缺少返回按钮"实为"看不见"）、第 4 条"所有页面默认 dark"未达成、主页"继续/读取存档进度"不可读。
- 完成定义：三层暗层按 §1 现行值实现并覆盖全部系统级页面（主页/存档/章节-地图/画廊/设置）；splash 类与 story 类同步核对是否也停留在旧值；改完自测返回按钮在亮背景上清晰可见。
- 最新更新时间：2026-07-26

### TASK-20260726-003
- 标题：Web 主页"存档进度"按钮无存档时被 disabled（Ant 第 6 条真因）
- 负责人：Wewe（Web）
- 状态：ready
- 优先级：P1
- 说明：`web/src/ui/screens/StartScreen.js:77` 无 autoSave 时 `this._savesBtn.disabled = true`，导致首次进入点击毫无反应。前一轮修复（给 `system-bg` 加 `pointer-events:none`）打在无关图层上，故现象未变。按 Interaction §23 空状态设计，存档页应可进入并显示空槽位，而非禁用入口。
- 完成定义：无存档时"存档进度"仍可打开，页面显示空状态；实测点击生效。
- 最新更新时间：2026-07-26

### TASK-20260726-004
- 标题：UI 体检脚本（机械对账，取代截图对比）
- 负责人：feibo（设计）/ Wewe（实现）
- 状态：pending
- 优先级：P1
- 说明：把 MinSpec 中**可判定**的项（暗层 token 值、字号、行高、间距、颜色、元素存在性、按钮 disabled/可点性、层级遮挡）写成断言，用无头浏览器读运行时 computed style 逐条核对，输出通过/不通过清单。定位：**机械对账，非品味判断**——AI 在此类事上可靠，在"看图判断好不好看"上不可靠。今日三个根因（暗层旧值、按钮 disabled、返回按钮不可见）体检脚本均可在提交前拦下。
- 完成定义：脚本可跑、清单可读；交付规则改为"体检不过不得报完成"。
- 最新更新时间：2026-07-26
