...
- 状态：ready
- 优先级：P0
- 现象（Ant 实机/浏览器反馈）：系统级页面几乎没有压暗，白色返回按钮与次要文字糊在亮色背景上看不见；主页"继续/读取存档进度"不可读。
- 范围：`web/styles/` 暗层相关实现，覆盖全部系统级页面；同时核对 splash 类与 story 类暗层是否同样滞后。**不碰 Android、story-data、资源文件。**
- 落地依据：**`authority/ui/XoXo_UI_Final_MinSpec_20260712.md` §1（暗层系统）**。数值一律以该节原文为准，本条目不提供任何数值。
- 现状线索（feibo【已验证】，仅供定位，**不得据此直接改，必须回权威取值**）：`web/styles/tokens.css` 中系统级暗层 token 停留在 §1 修订前的旧口径，且缺少其中一层。成因：90 项对齐（`e728137`）做在 §1 修订之前，权威更新后无人回归。
- 完成定义：§1 全部要求落地并自行在浏览器复现证明"返回按钮在亮背景上清晰可见"；pre-flight 先报 §1 是否有缺失/歧义。
- 最新更新时间：2026-07-26
...
- 落地依据：**`authority/interaction/NagisHeart_Interaction_Design_v1_0.md` §13（存档页交互）、§23（空状态设计）**。入口在无存档时应表现为何种状态，以该两节原文为准；若两节未明确规定，pre-flight 报缺失，等裁决，**不许自行决定**。
- 完成定义：按权威规定实现并自行复现证明现象消失；pre-flight 先报上述两节是否覆盖该场景。
- 最新更新时间：2026-07-26
