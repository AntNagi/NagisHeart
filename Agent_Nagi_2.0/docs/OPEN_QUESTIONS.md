...
## 2026-08-22 F25 部分完成 —— embedding 基础设施就绪，**调用未验证**

Ant 指示继续，但当日 Gemini 免费额度已用尽（3 轮全量烘焙 + 2 轮 30 条 Eval），
故只做**不需要实际调用**的部分。**以下明确区分已验证与未验证。**

### 【已验证】混合向量空间的行为

实测三条记忆（`embed-v1` 4 维 / `embed-v2` 8 维 / 无向量），用 `embed-v2` 查询：

```text
纯词面查询（无向量）  → 命中 3 条
用 embed-v2 查（8维） → 命中 2 条，**old 消失**
```

**过滤本身是对的**（V4 §8.2「禁止混合向量空间检索」），
但它**完全静默**——凪会突然想不起一批事，日志里什么都没有。

> 更正：我一度以为「查询不带模型标识时一律放行」是漏洞。
> 再读一遍不是——那是纯词面检索的场景，没有向量参与，谈不上混合空间。

新增 `findStaleEmbeddings()` + 启动时告警，让这类静默退化显形。
V4 §8.2 要求「模型变化时后台重建全部向量」，**重建尚未实现**，
当前只做到「至少喊一声」。4 条回归测试锁住行为。

### 【已验证】维度不匹配立刻失败

`OpenAICompatibleEmbeddingProvider` 在收到与配置不符的维度时**直接抛错**，
不接受、不推断。理由：从响应推断维度，一旦与库里既有向量不一致，
就静默产生了混合向量空间。**维度必须显式配置**（`NAGI_EMBEDDING_DIM`）。

### 【未验证】实际调用

**没有跑通过一次真实的 embedding 请求。** 以下全部属于【推断】：
- 端点路径（Gemini `/v1beta/openai/embeddings`、方舟 `/api/v3/embeddings`）
- 请求体格式（`{model, input: string[]}`）与响应结构（`data[].embedding`）
- 各家模型的实际维度 ⇒ `providers.yaml` 里 `modelId` / `dimension` 仍是 `TBD_待实测`

⇒ **配额恢复后必须先实跑一次**，再谈接进检索链路。
`active: not_configured`，未配置时检索退回纯词面，现有功能不受影响。

### 仍未做

1. **向量重建**（V4 §8.2 硬要求）。换模型后需后台重跑全部记忆的 embedding。
2. **接进检索链路**。`retrieveContext` 目前不传 `query.embedding`，
   即便 provider 配好了也还是纯词面。要等实际调用验证过再接，
   否则接上一个没验证过的东西反而会掩盖问题。

---

## 2026-08-22 F25 接线完成 —— embedding 实调已验证，端到端待额度恢复（Claude）

### F25 状态推进：基础设施就绪 → **接线完成，厂商调用已实测**

**已验证（实调 Gemini）**
- 端点 `https://generativelanguage.googleapis.com/v1beta/openai/embeddings`
  `gemini-embedding-001` 返回 **3072 维**。`config/providers.yaml` 的两处 `TBD_待实测` 已按实测填入，
  `active` 从 `not_configured` 改为 `google_gemini`。火山 Ark 那组仍是 `TBD`，标注为【推断】。
- 语义分辨力实测：查询「他是不是很懒」
  - 对「凪怕麻烦，不愿意动」余弦 **0.713**
  - 对「世界杯决赛的比分」余弦 **0.524**

  二者与查询**零个共同 bigram**，词面分恒为 0——这正是引入向量要解决的那类查询。

**已接线**
- 查询侧：`retrieveContext` 现在带 `embedding` + `embeddingModel`。拿不到向量则只传 `text`，退回词面。
- 写入侧：`commitTurn` 在落库前给 memoryDraft 批量算向量。
- 两侧共用 `embedOrDegrade`，失败**留声后降级**，不拖垮整轮对话。

### F30 — 混合向量空间下，无关记忆会压过真相关记忆 —— 【已验证】

接线后立刻暴露的问题，也是我**先写错了注释、再实测纠正**的一条。

我最初写的是「有向量的记忆恒赢过无向量的」。**这是错的**，实测反例：
无向量但词面全中的记忆语义分是 1.0，照样赢（总分 0.853 vs 0.723）。

真实的失效形状是另一种：

| 记忆 | 向量 | 与查询「他是不是很懒」的关系 | 总分 |
|---|---|---|---|
| 「世界杯决赛的比分」 | 有 | **无关** | **0.723** |
| 「凪怕麻烦，不愿意动」 | 无 | **真相关** | 0.336 |

机制：`scoreMemory` 取 `semantic = max(余弦, 词面)`。余弦经 `(x+1)/2` 归一后，
**毫不相关**的中文句子也有 ~0.75 的地板；而无向量的记忆只能拿词面分，
中文改写查询下常常正好是 0。

所以准确的说法是「**改写型查询下无向量必输**」，不是「有向量恒赢」。

**为什么这在本项目里必然发生**：canon 记忆从 JSON 加载，天生无向量；
live 记忆现在写入时就带向量。症状将是「凪记得你昨天说的话，却忘了自己的剧情」。

两条形状都已锁进 `packages/core/test/core.test.ts`「混合向量空间」——
只留注释挡不住以后有人改打分公式。

### F25 后续 —— 向量重建已实现（V4 §8.2）

`packages/server/src/vector-rebuild.ts`。同时覆盖两种成因：
换了 embedding 模型（旧向量被静默过滤）、以及压根没有向量（canon）。

- 分批（默认 16 条/批），启动时**不阻塞**服务。
- 某批失败**即停不重试**——继续循环只会拿同一批反复撞同一个错误把额度烧光；
  已落库的部分是真断点，下次启动续跑。
- **不使用请求方的 BYOK key** 跑全库任务：那是花别人的额度，且 BYOK key 不该活过单次请求（域 B 红线）。
  未配 `NAGI_DEV_LLM_KEY` 时跳过并留声。
- 抽成独立模块而非模块级副作用，就是为了能测。8 条测试覆盖：分批收敛、
  换模型替换而非并存、失败即停（断言**调用次数**，否则无限重试的实现也能过）、
  真 SQL 的筛选与 limit、以及「补齐后真相关记忆重新排到前面」。

### 仍未验证 —— **端到端 live 未跑**

Gemini 免费额度在跑 F26 校准时耗尽（该次校准也因此中断，20 分钟零输出，已终止）。
未验证的是：

1. 真实对话中向量写入库、下一轮检索命中的完整链路
2. `rebuildVectors` 打真实端点的表现（批量 16 条是否撞 Gemini 单请求上限）
3. 3072 维 × 51 条 canon 的落库体积与检索耗时

前两项的**逻辑**已用假 embedding 测过，**厂商调用**已单独实调过，
但二者接起来跑一遍仍是必要的——中间还有 SQLite 的 JSON 序列化与
`INSERT OR REPLACE` 的实际行为。额度恢复后补。
...
## 2026-08-22 端到端 live 验证通过 + 配额实况（Claude）

### 配额实况 —— 我此前说的「Gemini 额度用完」不准确

实测三个端点，结论各不相同：

| 端点 / 模型 | 状态 | 依据 |
|---|---|---|
| chat `gemini-flash-lite-latest` | ❌ **每日**额度耗尽 | `limit: 500, model: gemini-3.5-flash-lite`，等 45 秒重试仍 429 ⇒ 是 RPD 不是 RPM |
| chat `gemini-flash-latest` | ✅ 可用 | 免费额度**按解析后的具体模型分别计**，换模型即得新额度 |
| embeddings `gemini-embedding-001` | ✅ 可用 | 与 chat **分开计配额**，chat 429 时它照常工作 |
| 火山方舟（豆包） | ❌ `AccountOverdueError` | 账户欠费，需 Ant 充值 |

两个此前不知道的事实：
1. `latest` 别名会解析到具体版本（`gemini-flash-lite-latest` → `gemini-3.5-flash-lite`），**配额算在解析后的模型上**。
2. chat 与 embedding 配额独立 ⇒ chat 打不动时仍可做记忆侧的工作。

`.env` 已把 main/aux 切到 `gemini-flash-latest`（`.env` 已 gitignore，不入库）。
⚠ 该模型实测**不稳定**：三次请求里两次返回 503，需重试才成功。调试期可接受，
但这会污染 eval 结果——F28 已经踩过「限流被误记成尺子判错」的坑，重跑 F26 时要留意区分。

### 端到端 live 已通过（F25 待验三项全部消除）

全新库、真服务（`PORT=4602`）、两轮真实对话：

**第 1 轮**「我今天换了新工作，在一家游戏公司做策划」
```
凪: 做游戏的？\n……那以后
retrieve_context  → 8 memories（topK 配置生效，此前硬编码 4）
assemble_context  → 8587 tokens | 20 blocks (…memory:8) | dropped 0
extract_effects   → 1 drafts
```

**第 2 轮**「你还记得我做什么工作吗」
```
凪: 游戏策划。\n不是刚说的。
retrieve_context → 9 memories，末位 e2e:2026-08-22T13:10:48.234Z:0(0.837)
```
⇒ 上一轮写入的 live 记忆被检索到并被正确使用。**记忆闭环成立**。

topK 4→8 后 Context 为 8,587 / 20,000 token，预算充裕（此前担心的挤爆没有发生）。

### F32 — 检索没有相关性下限，永远塞满 topK 条 —— 【已验证】

**先纠正我自己**：我看到分数挤在 0.885–0.902（跨度 0.006）后，一度判断
「排序几乎不由相关性决定」。**这个判断是错的**，实测排序是对的：

| 查询 | 首位命中 | 分数 |
|---|---|---|
| 「世界杯决赛那个关键进球」 | `dream_match`（决赛夜晚）0.922、`wc_keygoal` 0.920 | 均高于基线 |
| 「我们住的那个家」 | `dream_home`（买下花园别墅作为家）0.901 | 正确 |
| 「你和凛的关系怎么样」 | 无相关记忆 → 返回基线噪声 0.899 | — |

真正的问题窄得多：**强命中与「什么都不相关」之间只差约 0.023**
（0.922 vs 0.899），而且没有任何下限——第三个查询库里根本没有对应记忆，
检索仍然返回 8 条、每条 ~0.89，调用方无从分辨「找到了」与「没找到」。

后果：凪每轮都被塞进 8 条 canon，相关与否不论。表现会是
**答非所问式的联想**——你问 A，他扯到剧情里的 B。

⇒ 需要一个最低分阈值：低于它就少返回或不返回。但**阈值定多少是手感问题**，
且 `config/runtime.yaml` 的 `retrieval` 段目前没有这个键。
按契约「看不到明确规定的不许按理解补」，**留待 Ant 定**。
建议先按 0.90 试（基线 ~0.89 之上一点），人工体验后再调。

注：词面模式下同样压缩（跨度 0.024），所以这不是向量引入的新问题，
只是向量把跨度进一步压到 0.006，让阈值更难定。

---

## 2026-08-22 F26 校准跑通 15/15 + Gemini 免费额度实况（Claude）

### F26 已结清 —— 评分器校准 15/15

```
judgeCalibration: { total: 15, passed: 15, unavailable: 0, failures: [] }
guardEval:        mustBlock 9/9, mustPass 10/10
```

曾判错的 `Rel §10.3「你正在控制我。」`（原给 4 分）在集合里（第 35 行），
本轮通过。补入「设计文档语言」作为第八类失败类型的修改生效。

⚠ **这把尺子是 `gemini-3.1-flash-lite`，不是最终配置的 `doubao-seed-2-0-mini-260215`。**
换回豆包后**必须重跑校准**——校准量的就是「这个模型当尺子准不准」，
换模型即等于换尺子，旧读数不可继承。

### 已修 —— 校准不再被角色 Eval 绑架

原实现把校准嵌在角色 Eval 的分支里、且在它**之后**。
想跑 15 次调用的校准，就得先烧掉角色 Eval 的 ~90 次——免费额度下跑不完。
实测两次尝试分别耗时 20 分钟、10 分钟，都没跑到校准就被我掐掉。

新增 `NAGI_EVAL_ROLE=0` 跳过角色 Eval。除了省额度，还有第二个理由：
main 位换成调试模型时（豆包欠费期间临时用 Gemini），
**角色 Eval 的分数不代表实际配置**，跑了也不能信；而校准照样有效。

```bash
NAGI_EVAL_ROLE=0 NAGI_EVAL_CALIBRATE=1 pnpm run eval
```

### 已修 —— eval 熔断

`withRetry` 的退避（3+8+20+45=76 秒）是给**瞬时**限流设计的。
撞上**每日**额度耗尽时重试永远不会成功，却仍在每条用例上烧满 76 秒。
现在连续 3 条厂商侧不可用即中止，错误里直接写明「多半是每日额度、
换 `NAGI_LLM_MODEL` 即可获得新额度」。

产出一份全是 error 的报告**比没有报告更危险**——它看起来像"跑过了"。

### Gemini 免费额度实况 —— 按解析后的具体模型分别计，且额度差异极大

| 配置写法 | 实际解析到 | 每日上限 | 实测 |
|---|---|---|---|
| `gemini-flash-lite-latest` | `gemini-3.5-flash-lite` | **500** | 已耗尽 |
| `gemini-flash-latest` | `gemini-3.7-flash` | **20** | 20 次即耗尽 |
| `gemini-3.6-flash` | — | ? | 可用 |
| `gemini-3.1-flash-lite` | — | ? | 可用（本轮校准用它） |
| `gemini-3-flash-preview` | — | ? | 可用 |
| `gemma-4-31b-it` | — | ? | 可用 |
| `gemini-2.5-flash` / `-lite` / `2.0-flash` | — | — | **404**（在 models 列表里但 OpenAI 兼容端点不认） |
| embeddings `gemini-embedding-001` | — | ? | 可用，**与 chat 分开计** |

两条实用结论：
1. **别用 `-latest` 别名**。它解析到哪个版本不受控，而额度算在解析后的模型上——
   `gemini-flash-latest` 只有 20 次/天，几轮对话就没了，且失败时看不出是这个原因。
2. 撞 429 时**换模型即可续命**，不必等到第二天。当前 `.env` 已改为
   main=`gemini-3.6-flash`、aux=`gemini-3.1-flash-lite`。

这些都是**调试期的权宜**。首发选型仍是豆包（`NRH-20260820-019`），
账户充值后按 `.env` 里的注释块切回，并**重跑校准**。

---

## 2026-08-22 检索权重接线完成（NRH-20260822-023，Ant 裁 A）+ F33（Claude）

### 已修 —— `retrieval.weights` 接线，行为逐位不变

配置改写成代码实际在用的五维（`semantic / recency / salience / confidence / canonBoost`），
并真正接进 `scoreMemory`。`emotionIntensity` 移除——记忆结构里没有这个字段，
要做它属人设决定，已按裁决改列为独立需求。

**落地判据已验证**：同一组记忆、同一组查询，改前改后分数逐位相同。

```
词面查询  a 0.984000 / c 0.432000 / b 0.333355 / d 0.259207
向量查询  c 0.848000 / a 0.464000 / b 0.333355 / d 0.259207
```

`canonBoost` 在类型与注释里都明确标为**加项不是权重**——
混进「权重」里会让人以为它参与归一，改起来出错。

权重**整组回落**而非逐项：任一项缺失或非法即整组用默认值。
半套配置值配半套默认值得到的是谁都没标定过的第三套比例，
那种错不报警，只会让凪想起来的东西慢慢变怪。

单测锁死「配置值 == 原硬编码值」。日后有人改 `runtime.yaml` 里这几个数，
这条测试会红——**红得对**：它在提醒「你正在改凪的记忆偏好，这是人设决定」。

### F33 — 守卫不拦「结构性垃圾」，只拦「不像凪」 —— 【已验证】

用 `gemini-3.6-flash` 连发 4 条「在干嘛」，**4 条全部带结构性垃圾，
且 4 条全部通过 hard_guard 与 soft_judge**：
...
`output_guard.md` 的 10 条规则全部针对**像不像凪**
（心理咨询腔、热血宣言、霸总口吻、设计文档语言），
**没有任何一条检查「这到底是不是一句中文台词」**——
提示词泄漏、英文、markdown 残片、模型草稿，一律放行。

这类输出会**直接出现在 Ant 的聊天界面里**，且是最刺眼的那种穿帮。

#### 根因已修（provider 侧）

思考链泄漏进正文。Gemini 默认 `reasoning_effort` 下会把草稿吐进 content。
按既有的厂商参数模式（豆包 `thinking: {type: disabled}`）加了 Gemini 规则：
main 用 `reasoning_effort: "low"`，aux 用 `"minimal"`。
（`"none"` 会被拒：400 INVALID_ARGUMENT。）
...
⚠ 注意豆包 main 位**仍不关思考**——那是质量取舍，
且 `NRH-20260821-2037` 的 30 条对抗用例即在开启状态下标定。Gemini 关它是因为**泄漏**，两件事理由不同，别混为一谈。

#### 兜底已加（归一层）

`normalizeOutput` 现在剥掉 `*` `#` 反引号。归一层的职责本就是吸收厂商格式差异，
而不该指望每家厂商都有关思考的开关。用上面四条**真实采样**做的测试。

不碰 `_`：资源模板里有 `{{player_name}}` 这类占位符，误伤代价更大。
不碰省略号：降级模板第一句就是「……好麻烦。」，误伤等于把凪的口癖删了。

#### 仍未做 —— **需 Ant 裁决**

守卫本身**没有加规则**。要不要加一条「结构性完整性」判据（例如：
台词里出现连续拉丁字母、或中文字符占比过低即判失败并重生成），
属于**给权威加规则**，按契约 worker 无权自决。

我的看法：这不是人设规则，而是「模型这次有没有正常返回」的健全性检查，
与已有的长度上限同类（长度上限也不在 Bible 里，是按 V17 标定的工程判据）。
但归一层兜底之后现象已不外泄，**不紧急**，建议与 F15b、beat 超限提 block
一并裁，不单独开一轮。

---

## 2026-08-22 客户端接入准备（Chatbox，Ant 选型）

### F34 — 缺 `GET /v1/models`，任何现成客户端都进不到聊天界面 —— 【已验证，已修】

`http.ts` 只实现了 `POST /v1/chat/completions`。而**现成客户端连上来第一件事
是拉 `/v1/models` 填模型下拉框**，拉不到会停在「获取模型失败」，
根本走不到发消息那一步。

这类失败极难自查：服务端日志里只有一条 404，客户端只说「连接失败」，
看起来像是客户端自己的毛病。8/27–8/28 排期里的「客户端配置接入」
本来会整个卡在这里。

已补。只暴露一个 id `nagi`——按 V4 §10，客户端不该知道也不该选择厂商模型，
main / aux 由服务端按能力位决定。这同时挡住了「用户在客户端里把模型改成别的」，
那会绕开我们对 main 位的全部标定。

### BYOK 与鉴权互斥，接现成客户端时必须二选一 —— 【已验证】

现成客户端的「API Key」栏是**必填**的，且只会发成 `Authorization: Bearer`。
而 `resolveLlmKey` 的口径是：未配 `NAGI_AUTH_TOKEN` 时 Bearer 当模型 key，
配了则 Bearer 归鉴权。两条路互斥：

| 配置 | 客户端 API Key 栏填什么 | 模型 key 在哪 |
|---|---|---|
| 不配 `NAGI_AUTH_TOKEN` | **真的模型 key** | 客户端（含手机）上 |
| 配 `NAGI_AUTH_TOKEN` | 那串 token | 只在服务端 `NAGI_DEV_LLM_KEY` |

**取后者**：模型 key 不离开这台机器，手机上不存任何厂商凭证。
`.env` 已加 `NAGI_AUTH_TOKEN`（随机串，`.env` 已 gitignore）。

实测（`PORT=8787`）：无 token → 401；带 token → `/v1/models` 与流式聊天均正常。

### 已加 —— `NAGI_HOST`，绑非回环地址强制要求鉴权

服务端此前**写死绑 `127.0.0.1`**，手机连不上（同 wifi 也不行），
且失败时服务端日志一片空白。现在可用 `NAGI_HOST` 配置，默认仍是 `127.0.0.1`。

**绑非回环地址而未配 `NAGI_AUTH_TOKEN` 时拒绝启动**：不配 token 时
`Authorization` 被当作 BYOK key，等于任何人都能读走 `/api/history`（完整对话记录）、
`/api/state`（关系数值）、`/api/save/export`（整份存档）。
咖啡馆或公司网络下这就是把私人对话摊开——宁可起不来，也不要静默暴露。

### 仍然解决不了的：多气泡

OpenAI 流式协议里 chunk 在客户端会**拼成一条消息**。换任何标准客户端都一样，
凪的多句台词只能靠换行分行，不会变成多个气泡。要真的多气泡就得 fork 客户端，
而 V4 §14.4 明确把「对开源聊天客户端做大规模二次开发」列为后置。

---

## 2026-08-23 F36 — 聊天路径没有重试，一次瞬时限流直接变成使用者眼前的错误 —— 【已验证，已修】

**现象**（Ant 实际使用中撞上）：连聊两轮后第三轮返回
`provider request failed (429) for model gemini-3-flash-preview`，
错误原样显示在聊天气泡里。

**关键事实**：这些 429 **不是每日额度耗尽，是短窗限流，会自己恢复**。
撞上后立刻探测五个模型，包括此前判为「每日耗尽」的
`gemini-flash-lite-latest` 与 `gemini-3.6-flash`，**全部可用**。

⇒ 我此前把「等 45 秒重试仍 429」判为 RPD 耗尽，那个推断**在这个语境下不成立**：
窗口比 45 秒长，但远不到一天。已在此更正。

**根因**：`evals/role-eval.ts` 早就有 `withRetry`（3s/8s/20s/45s 退避），
但**服务端聊天路径一次重试都没有**。于是出现「Eval 全绿、实际聊两句就报错」
这种矛盾观感——两条路走的韧性策略完全不同，而只有 Eval 那条被检验过。

**已修**：重试下沉到 `OpenAICompatibleProvider.complete`，
main / aux 两个能力位同时受益。

- 退避 1s / 3s / 8s，总计约 12 秒。**再长不如直接报错让人重发**——
  对话是同步交互，等待本身也是一种失败。
- **只重试瞬时故障**（429/5xx/超时/网络）。鉴权失败、欠费、请求格式错误立刻抛：
  重试只会让使用者多等十几秒再看到同一条错误。
- 日志只打印 message 不打印 error 对象（域 B 红线：key 不进日志）。

三条单测锁住：429 后重试成功、401 立刻抛不重试、
一直 429 则在退避用尽后抛出且**断言调用次数为 4**——
少了最后这条断言，无限重试的实现也能过前两条。

### ⚠ 顺带修正一处此前的记录

`config/providers.yaml` 与 OPEN_QUESTIONS 里「gemini-flash-lite-latest 每日额度耗尽」
的结论应降级为**【推断】**：当时只验证了「等 45 秒仍 429」，
而本次实测表明该模型在数小时后恢复可用。真实的窗口长度**未测定**。
