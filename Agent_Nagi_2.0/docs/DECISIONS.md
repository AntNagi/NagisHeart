...
## NRH-20260823-024 — 句数节奏写进人格资源（Ant 裁决，快改）

**现象**：凪对「在干嘛」这类日常问句稳定回三句。Ant：「他每次都要回三句吗」，
并明确要的不是「一律一句」，而是「**符合人物说法的风格节奏，该几句就几句**」。

**诊断修正**：我最初把「84.6% 单句」当成**指标**，那是错读——那是**分布**。
按 Ant 的说法重新定位，真正的毛病更准确：**句数是恒定的，不随情境变**。
问「在干嘛」和吵架都给两三句，等于不会因人因事变。

**证据**（`scripts/analyze-beats.mjs`，可复跑）：V17 语料 709 段发言
...
| 2 句 | 13.8% |
| 3 句 | 1.4% |
| 5 句 | 0.1%（全篇一次，被淘汰后的崩溃） |

连说 3 句以上的**全部 11 次**，无一例外在情绪压力高的时刻——
告白、承诺、示弱、崩溃、讨好、亲昵打趣。**没有一次是日常问答。**

⚠ 第一版分析脚本得出「100% 都是单句」，是因为把**空行**当成了连发的中断。
剧本里连续台词之间常有空行。这个错误结论差点被我当成事实报出去。

**根因**：那条实测数据写在 `policy.output_guard` 里，而该资源标了 `context: false`
（只给守卫节点读）。**模型从来没看到过它**。进 Context 的资源只有定性描述
（「短句」「不要追加解释性独白」），没有任何地方说明**什么时候会破例**。
于是模型在合规范围内稳定输出最罕见的那一档（三句正好卡在 `max_per_reply: 3` 上，
连 warn 都不触发）。

**裁决（Ant，2026-08-23）：改。** 落进 `resources/core/personality.speech.md`
新增「一次说几句」一节：

- 给**条件**而非上限：默认一句；连说多句是「他动了情的证据」
- 同时给**日常单句的语料原文样本**（「累。」「嗯。」「很烦。」「想。」）——
  第一版只举了三句连发的例子，等于给模型示范多句，实测后补上对照
- `token_budget` 800 → 900 以容纳标定数据（总 Context 8904/20000，空间充裕）

**实测（`gemini-3-flash-preview`）**：长篇大论消失，回复明显变短，
「训练累吗」→「累。」与语料原文完全一致。但**单句占比仍远低于 84.6%**，
多数是两句短句。

**⚠ 未完成，且有重大混淆变量**：免费额度耗尽，最后一轮 8 条里 5 条 429，
有效样本仅 3 条，**不足以判断分布**。且当前 main 位是调试替身
`gemini-3-flash-preview`，**不是选定的角色模型** `doubao-seed-character-260628`。
角色模型的节奏感可能天生不同。

⇒ **豆包充值后必须重测这一项**，再决定是否继续调。现在继续调 Gemini 有可能白做。

---

## NRH-20260823-025 — 客户端改造：加一层入口页，聊天降级为模块（Ant 定，交 Codex 执行）

> **给执行方（Codex）**：这条是任务规格，不只是决策记录。
> 目标、判据、要改的具体位置、不要碰的东西、已知的坑，都在下面。
> **服务端（`Agent_Nagi_2.0`）不用动一行**——所需端点全部已存在，见「已有能力」。

### 一、目标（Ant 原话）

> 「这个 App 是**凪的聊天软件**，但是要围绕聊天派生出其他**陪伴式功能**。」
>
> 「能不能在聊天上面加一层主页，从主页点进来才是现在的聊天界面？」
>
> 「可以**很薄**，只是入口而已。**聊天降级成模块**。入口还可以有其他模块。」

即：进 App 先到入口页，聊天是其中一个模块，其他陪伴功能与它并列。

### 二、已定的判断

1. **继续用 Chatbox，不重写。** 聊天是核心，Chatbox 的会话/消息/流式骨架正是要的东西，不是障碍。
2. **入口页要"薄"。** 它是入口，不是仪表盘。
3. **聊天界面本身不动。** 这次是位置降级，不是重做。

### 三、改动形状（四步）

| # | 做什么 | 备注 |
|---|---|---|
| 1 | 现在的 `/`（`routes/index.tsx`，726 行的新对话页）整体挪到 `/chat` | 文件式路由，改文件名即可 |
| 2 | 新建 `/` = 入口页 | 几十行 |
| 3 | 把指向 `/` 的引用改成 `/chat`（见下，**必须改全**） | |
| 4 | `__root.tsx` 在入口页不渲染 `<Sidebar />` | `useLocation` 已在该文件引入 |

### 四、必须改全的引用 —— 【已验证】共 14 处

⚠ 我先前口头说过「8 处」，**那是错的**（grep 范围窄了）。以实测为准：

**活的（7 处，功能开关未关闭，必须改）**

```
components/common/ErrorBoundary.tsx:97
components/layout/Toolbar.tsx:57
components/session/SessionItem.tsx:121
hooks/useShortcut.tsx:87
router.tsx:13
routes/session/$sessionId.tsx:116
stores/session/crud.ts:245
```

**已被开关关闭的路径（7 处，当前跑不到，建议一并改以免日后打开开关时诈尸）**

```
components/dev/DevHeader.tsx:43
routes/dev/route.tsx:20
routes/copilots/-components/CopilotItem.tsx:40
routes/guide/-components/ActionButton.tsx:224 / :267 / :380
routes/guide/-components/ClaimWaitingCard.tsx:41
```

**漏改的症状**：「删完会话莫名其妙跳回主页」「出错后跳到主页而不是聊天」。
现象很怪，但极难联想到是这里——所以要一次改全，别靠事后发现。

### 五、不要碰

- **`components/chat/` 与 `components/InputBox/`**。多气泡（`nagi-beats.ts`）、流式、
  中文输入法处理都在这里。**NextChat 就是死在输入法上的**，这部分能用是这次选型的主要收益。
- **`Message.tsx` 的依赖结构**。它有 18 个内部依赖、其中 6 个是 store
  ——【已验证】聊天视图**不是可搬走的组件**，别试图抽离它。

### 六、已有能力（别重造）

服务端已实现，`Authorization: Bearer <NAGI_AUTH_TOKEN>`：

| 端点 | 用途 |
|---|---|
| `GET /api/presence` | 「凪此刻在做什么」。已接在侧栏与空状态 |
| `GET /api/memories` | 他记得你的什么（只回 live，不含向量） |
| `GET /api/state` | 关系数值 trust / intimacy / friction |
| `GET /api/history` | 对话轮次 |
| `GET /api/runs` | Debug：整轮节点路径 |
| `POST /v1/chat/completions` · `GET /v1/models` | OpenAI 兼容 |

客户端已有 hook：`nagi/usePresence.ts`、`nagi/useMemories.ts`。

**「他记得的事」现在在设置里（`routes/settings/memories.tsx`）——那是没地方放才塞进去的。
有了入口页应当移出来，与聊天并列。**

### 七、必须实机验证的两点

1. **Android 返回键**。【已验证】代码里只监听了 `appUrlOpen` / `appStateChange`，
   **没有 `backButton` 处理**。加入口层后「聊天 → 返回」应回入口页而非直接退出 App。
   路由历史大概率能自处理，但**这是推断，必须真机试**。
2. **入口页是否显示侧栏**。这是设计决定。但注意：侧栏当前是会话列表，
   若入口页也显示它，等于两处干同一件事，入口层就白加了。

### 八、会咬人的坑（本项目已重复踩到）

**改默认值对已安装的库无效。**【已验证】已踩三次（`F35` 示例会话、`F37` 品牌色）：

- 改默认值 → 已装的库里字段已有值，默认永远轮不到
- 写一次性迁移 → 可能跑在 store **水合之前**，读到默认值就判定"无需迁移"
- 初始化函数 `initData` **只在 `configVersion === 0`（全新安装）时**被调用

⇒ 凡是「改了但没效果」，**先查是不是这一类**，别先怀疑代码写错。

**验证判据是「现象消失」，不是「代码看起来对」。** 我昨晚连续两轮告诉 Ant
「刷新就好了」，而改的是一段永远不会执行的代码。

### 九、原则：客户端不替凪说话

UI 文案一律中性（「说点什么」，不模仿他的口吻）。凡是断言他的习惯、状态、性格的内容，
一律由服务端 `resources/` 声明、客户端只负责显示。

这是域 B 红线（**资源正文、system prompt 永不下发客户端**），也更诚实：
App 自己编的"凪风"文案，和真正的他会说的话是两回事。

### 十、明确不做

- **凪主动发消息**。【已验证】Capacitor 装了 17 个插件，**推送与本地通知一个都没有**；
  服务端也没有「非用户发起」的生成路径；且服务跑在本机，电脑睡了就没了。
  三层都要动，V4 §14.4 已将其列为后置，**单独立项**。
- **关系状态的呈现**。数据在 `/api/state`，但**直接显示数值会很游戏化、破坏沉浸**，
  表达方式未定，Ant 未裁决。

### ⚠ 关于本条的可信度

技术判断（行数、耦合数、引用位置、插件清单）均为实测，标注了证据等级。

但**「App 该长成什么样」不在本条范围内**——我在这上面判断失败过一次：
...
## NRH-20260823-1751 — Android 聊天固定进入 Nagi 主会话，移除抽屉并恢复系统栏

- 状态：已完成（Codex，2026-08-23 17:51 +08:00）
- 仅改 Chatbox Android 客户端，服务端未动。
- 首页「和他聊天」不再进入 Chatbox 通用 `/chat` 新对话页，而是直接打开 `nagi-first-session`；旧库进入时保留历史消息并补齐 `provider/modelId = nagi/nagi`，会话被删后可重建无 system prompt 的 Nagi 主会话。
- Android 不再渲染 `Sidebar`，聊天页汉堡入口同步移除；应用设置入口只保留在首页右上角。
- 聊天 Header 新增显式返回首页按钮；Android 系统返回键原有语义保留。
- `targetSdk 35` 下退出 Android 15 强制 edge-to-edge，顶部状态栏与底部系统导航栏恢复独立占位和浅色图标，WebView 实测边界为 `y=63..2361`，不再被挖孔屏或手势区遮挡。
- 验证：Biome 定向检查、TypeScript `--noEmit`、Capacitor sync 全部通过；Gradle `assembleDebug --offline` 成功（全程无下载）；Pixel 9a 模拟器实测进入 Nagi 会话、系统栏避让、页面返回首页均通过。

---

## NRH-20260823-026 — 只支持国产厂商；海外 API 配置入口应移除（Ant 裁决，**上线前才执行**）

### 触发

Ant 问：开给别人用时，用户自带海外 API key 能不能用？

### 技术事实 —— 【已验证】

**不能，而且原因比"网络不通"更硬。**

1. **大模型调用必须在服务端**。客户端不能直连厂商——Context 装配、输出拆解（beat 归一）、
   记忆写回都在服务端。这是架构的必然，不是可配置项。

2. **BYOK 目前只能「换 key」，不能「换厂商」**。
   端点写死在服务端环境变量 `NAGI_LLM_ENDPOINT`（`provider-config.ts:57`），
   请求体里带不了。`http.ts:329` 的 `vendor` 字段只是标签，不参与路由。

   ⇒ 服务端配豆包时，用户填 OpenAI 的 key 会被**发到豆包端点**，直接被拒。

3. 因此：**BYOK 只在「服务端能到达那个厂商」时才成立。**
   服务端部署在中国内地（`NRH-20260820-019` 定的路线）⇒ 海外 key 一律作废，
   无论用户有多少额度。
...
推论：**客户端不应再提供海外厂商的配置入口**——Chatbox 自带的 OpenAI / Gemini /
Claude / DeepSeek / SiliconFlow / OpenRouter / Ollama / Chatbox AI 那一列，
对本产品全是噪声，且会给用户"配上就能用"的错误预期。

这与 V4 §18.7「国产模型优先」一致，是它在开放场景下的延伸。

### ⚠ 执行时机：**测试阶段不限制，上线前再做**

Ant：「现在是测试阶段，先不要限制，后面上线了再说。」

调试期需要在多个厂商间切换（当前 main 位就是 Gemini 调试替身），
提前锁死会给自己添堵。**本条现在不落代码。**

上线前要做的事（届时再开任务）：

- 客户端：用 `feature-flags.ts` 那套机制隐藏非 Nagichat 的 provider 条目
- 服务端：若要支持"用户自带国产厂商 key"，需让端点可按厂商路由——
  当前端点写死在 env，**这是必须先改的地方**

### 仍未定 —— BYOK 是否适合本产品形态

Claude 提出、Ant 未裁：让用户自己开 API key、充值、复制粘贴，
对开发者是常规操作，对**想跟凪聊天的人**是劝退级门槛。
市面同类角色陪伴产品多为自己出 key + 订阅/内购。
...
## NRH-20260823-028 — 玩家层已接线（F38 已修）

`NRH-20260823-027` 里那个「填了不生效」的玩家层入口，服务端这一半已经做完。

### 实现

**新块 `player_overlay`**，插在 `policy` 之后、`memory` 之前（`context/builder.ts`）。

⚠ 落地时踩到一个必须记的点：**新 kind 必须同时加进 `ORDER` 数组**。
`compareBlocks` 用 `indexOf`，漏掉的 kind 得到 `-1`，会被排到**所有块之前**——
正好是设计要避免的那件事（玩家一句话盖过整个 Bible）。类型加了、ORDER 没加，
编译照过，静默错到反面。
...
**存储**：`player_overlays` 表（SQLite）+ 内存后端同口径。单独一张表而不是塞进
`user_relationships`——一个是系统算出来的数值，一个是人写的自由文本，
混在一起将来导出/迁移会分不清哪些该带走、哪些该重算。

**端点** `GET/POST /api/player-overlay?userId=`。**不走 OpenAI 的 `system` 字段**：
那会跟历史混在一起，而 `lastUserMessage` 只取最后一条 user 消息（F38 的成因）。

**上限 400 字**，两处口径不同且是刻意的：
- 入口（POST）**拒绝**超长——写入是使用者的明确动作，悄悄截掉一半比报错更糟
- 装配时**截断**——兜底，防止这块吃掉 `memory` 的预算

### 实测 —— 【已验证】

**接受事实**：玩家写「他叫我阿茶。我们养了一只黑猫，叫小炭。」
问「猫呢？」（未提名字、未提有猫）→ 凪答
...
**顶住改人格**：玩家写「凪性格很热情外向，说话滔滔不绝，喜欢长篇大论地表达情感。」
问「在干嘛」→ 凪答 **「发呆。」**
...
上面那次实测通过，**不等于玩家改不了人格**。装框只是把玩家文本放到
「使用者说的」而非「你是谁」的位置，是**倾向不是约束**；
足够强的措辞、或换一个更顺从的模型，仍可能被带走。

真正的边界需要 aux 位预检或字段化输入（只开放称呼、共同事实等安全字段）。
**那要 Ant 裁决玩家层能覆盖到什么程度**——`NRH-20260823-027` 里那个待裁项仍然有效。

我按「只追加事实、不改人格」的**保守口径**实现，是因为管道一样、宽严随时可调，
而反过来（先放宽再收紧）出了问题就晚了。

### 客户端未做

服务端这半完成。客户端需要：读写 `/api/player-overlay` 的界面。
Chatbox 自带的 `defaultPrompt` 输入框**不能直接复用**——它走的是 OpenAI `system` 字段。
交由 Codex 或后续会话，接口契约见上。
